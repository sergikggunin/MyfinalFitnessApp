...
import com.example.myfinalfitnessapp.databinding.DaysListItemBinding
...
class DaysAdapter(var listener: Listener) : ListAdapter<DayModel, DaysAdapter.DayHolder>(MyComparator()) {
    ...
    class DayHolder(view: View) :  RecyclerView.ViewHolder(view) {
        ...
        fun setData(day: DayModel, listener: Listener) = with(binding) {
            ...
            tvName.text = name
            val exCounter =
                day.exercises.split(",").size.toString() + " " + root.context.getString(R.string.exercise) + root.context.getString(
                    R.string.exercise
                )
